package cordova-plugin-azureadb2c;
...
public class azureadb2c extends CordovaPlugin {
        ...
        cordova.getThreadPool().execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    try {
                        keyHashUrlFriendly = URLEncoder.encode(azureadb2c.this.keyHash, "UTF-8");
                    } catch(UnsupportedEncodingException e) {
                        azureadb2c.this.callbackContext.error(e.getMessage());
                    }
                    ...
                    try {
                        ...
                        data = "{\n" +
                                ...
                                "    \"redirect_uri\" : \"msauth://" + azureadb2c.this.activity.getApplicationContext().getPackageName() + "/" + keyHashUrlFriendly + "\",\n" +
                        ...
                        if (options.getString("accountMode").equals(SINGLE_ACCOUNT)) {
                            azureadb2c.this.appSingleClient = PublicClientApplication.createSingleAccountPublicClientApplication(context, config);
                            azureadb2c.this.accountMode = SINGLE_ACCOUNT;
                        } else {
                            azureadb2c.this.appMultipleClient = MultipleAccountPublicClientApplication.createMultipleAccountPublicClientApplication(context, config);
                            azureadb2c.this.accountMode = MULTIPLE_ACCOUNTS;
                        }
                        config.delete();
                        ArrayList<String> scopes = new ArrayList<String>();
                        ...
                        azureadb2c.this.scopes = scopes.toArray(new String[scopes.size()]);
                        azureadb2c.this.isInit = true;
                        azureadb2c.this.callbackContext.success();
            ...
            Logger.getInstance().setExternalLogger(new ILoggerCallback() {
                ...
                public void log(String tag, Logger.LogLevel logLevel, String message, boolean containsPII) {
                    try {
                        ...
                        azureadb2c.this.loggerPluginResult = new PluginResult(PluginResult.Status.OK, logEntry);
                        azureadb2c.this.loggerPluginResult.setKeepCallback(true);
                        azureadb2c.this.loggerCallbackContext.sendPluginResult(azureadb2c.this.loggerPluginResult);
                    } catch (JSONException e) {
                        azureadb2c.this.loggerCallbackContext.error(e.getMessage());
                    }
                }
            });
                ...
                public void run() {
                    ...
                    try {
                        if (SINGLE_ACCOUNT.equals(accountMode)) {
                            if (azureadb2c.this.appSingleClient.getCurrentAccount().getCurrentAccount() != null) {
                                accounts.put(getAccountObject(azureadb2c.this.appSingleClient.getCurrentAccount().getCurrentAccount()));
                            }
                        } else {
                            for (IAccount account : azureadb2c.this.appMultipleClient.getAccounts()) {
                                accounts.put(getAccountObject(account));
                            }
                        }
                        azureadb2c.this.callbackContext.success(accounts);
                    } catch (InterruptedException e) {
                        azureadb2c.this.callbackContext.error(e.getMessage());
                    } catch (MsalException e) {
                        azureadb2c.this.callbackContext.error(e.getMessage());
        ...
        if (this.checkConfigInit()) {
            if (SINGLE_ACCOUNT.equals(accountMode)) {
                cordova.getThreadPool().execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                            String authority = azureadb2c.this.appSingleClient.getConfiguration().getDefaultAuthority().getAuthorityURL().toString();
                            if (azureadb2c.this.appSingleClient.getCurrentAccount().getCurrentAccount() == null) {
                                azureadb2c.this.callbackContext.error("No account currently exists");
                            } else {
                                IAuthenticationResult silentAuthResult = azureadb2c.this.appSingleClient.acquireTokenSilent(azureadb2c.this.scopes, authority);
                                azureadb2c.this.callbackContext.success(getAuthResult(silentAuthResult));
                            }
                        } catch (InterruptedException e) {
                            azureadb2c.this.callbackContext.error(e.getMessage());
                        } catch (MsalException e) {
                ...
                cordova.getThreadPool().execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            boolean found = false;
                            for (IAccount search : azureadb2c.this.appMultipleClient.getAccounts()) {
                            ...
                            if (!found) {
                                azureadb2c.this.callbackContext.error("Account not found");
                                return;
                            }
                            String authority = azureadb2c.this.appMultipleClient.getConfiguration().getDefaultAuthority().getAuthorityURL().toString();
                            IAuthenticationResult result = azureadb2c.this.appMultipleClient.acquireTokenSilent(
                                    azureadb2c.this.scopes,
                                    azureadb2c.this.appMultipleClient.getAccount(account),
                                    authority
                            );
                            azureadb2c.this.callbackContext.success(getAuthResult(result));
                        } catch (InterruptedException e) {
                            azureadb2c.this.callbackContext.error(e.getMessage());
                        } catch (MsalException e) {
                            azureadb2c.this.callbackContext.error(e.getMessage());
                ...
                cordova.getThreadPool().execute(new Runnable() {
                    ...
                    public void run() {
                        AcquireTokenParameters.Builder params = new AcquireTokenParameters.Builder()
                                .startAuthorizationFromActivity(azureadb2c.this.activity)
                                .withScopes(Arrays.asList(azureadb2c.this.scopes))
                                .withOtherScopesToAuthorize(Arrays.asList(otherScopesToAuthorize))
                                .withPrompt(prompt)
                                .withCallback(new AuthenticationCallback() {
                                    @Override
                                    public void onCancel() {
                                        azureadb2c.this.callbackContext.error("Login cancelled.");
                                    }

                                    @Override
                                    public void onSuccess(IAuthenticationResult authenticationResult) {
                                        azureadb2c.this.callbackContext.success(getAuthResult(authenticationResult));
                                    }
                        ...
                        azureadb2c.this.appSingleClient.acquireToken(params.build());
                    }
                });
            } else {
                cordova.getThreadPool().execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        azureadb2c.this.appMultipleClient.acquireToken(params.build());
                    }
                });
            }
        }
        ...
        if (this.SINGLE_ACCOUNT.equals(this.accountMode)) {
            cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    try {
                        // Look for account first so we don't error out for one that doesn't exist
                        if(azureadb2c.this.appMultipleClient != null) {
                            boolean found = false;
                            for (IAccount search : azureadb2c.this.appMultipleClient.getAccounts()) {
                                if (search.getId().equals(account)) {
                                    found = true;
                        ...
                        if (azureadb2c.this.appSingleClient.getCurrentAccount().getCurrentAccount() != null) {
                            azureadb2c.this.appSingleClient.signOut(new ISingleAccountPublicClientApplication.SignOutCallback() {
                                @Override
            ...
            cordova.getThreadPool().execute(new Runnable() {
                ...
                public void run() {
                    try {
                        azureadb2c.this.appMultipleClient.removeAccount(azureadb2c.this.appMultipleClient.getAccount(account),
